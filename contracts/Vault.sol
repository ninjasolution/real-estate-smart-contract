...
contract Vault is  Ownable {
    ...
    function swapETHToToken(address token, uint256 amount) public authorized {
    ...
    function swapTokenToETH(address token, uint256 amount) public authorized {
    ...
    function tokenBalanceOf(address token) public view returns (uint256) {
        return IERC20(token).balanceOf(address(this));
    }

    receive() external payable {}

    function withdrawEntireETH(address receiver) public authorized {
    ...
    function withdrawOfETH(uint256 amount, address receiver) public authorized {
        require(address(this).balance >= amount, "Contract balance is not enough.");

        payable(receiver).transfer(amount);
    }

    function withdrawEntireToken(address token, address receiver) external authorized {
        IERC20(token).transfer(receiver, IERC20(token).balanceOf(address(this)));
    }

    function withdrawToken(address token, uint256 amount, address receiver) external authorized {
