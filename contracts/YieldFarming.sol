...
contract YieldFarming is Ownable, ReentrancyGuard {
    ...
    /**
       @dev To set the start and end blocks for each period
       @param _start start block number
       @param _end end block number
    */
    ...
    /** 
     * @dev to add rewards to staking contract
     * @param _rewardAmount amount which transfered to staking contract
    */
    ...
    /**
       @dev To reset the contract at the end of each period.
    */
    ...
    /**
     * @dev to initialize contract with start, end block and reward amount
     * @param _rewardAmount total amount for reward
     * @param _start start block number
     * @param _end end block number
    */
    ...
    /**
       @dev Function to update rewards and state parameters
    */
    ...
    /**
     * @dev to stake lp token to the contract
     */
    ...
    function _stake(address from, uint256 amount) private returns (bool) {
        ...
        if (!hasStaked[from]) {
            ...
            if (deposits[from].currentPeriod != period) {
                require(_renew(from), "Error renewing");
            } else {
    ...
    /**
     * @dev share percent in total reward amount
     */
    ...
    function _calculate(address from) private view returns (uint256) {
        ...
        uint256 newAccShare = currentAccShare.add((rewards.mul(1e6).div(stakedBalance)));
        uint256 amount = deposits[from].amount;
        uint256 rewDebt = amount.mul(userAccShare).div(1e6);
    ...
    function incBlock() external onlyOwner {
        startingBlock += 1;
    }
