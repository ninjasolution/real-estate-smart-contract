...
contract Testqwe is ERC20, Ownable {
    ...
    uint256 public _devFeePercent = 1; // 1%
    uint256 public _feeDivisor = 10000;  // 1% = 100
    ...
    constructor(address router) ERC20("Federation World Contact", "CWF") {
    ...
    function _transfer(
        ...
        if (msg.sender == owner()) {
            ...
            taxAmount = (amount * _feeSellTotal) / _feeDivisor;
            flag = Flag.Sell;
        } else if (from == address(swapRouter)) {
            taxAmount = (amount * _feeBuyTotal) / _feeDivisor;
            flag = Flag.Buy;
        } else {
            taxAmount = 0;
        }
    ...
    function setTaxWallets(address _charityWallet, address _devWallet) external onlyOwner {
    ...
    function updateTaxFee(uint16 buyFee, uint16 sellFee) external onlyOwner {
        ...
        require(buyFee <= maxTaxFee && sellFee <= maxTaxFee, "CWF: Exceed Max Tax fee");
        _feeBuyTotal = buyFee;
